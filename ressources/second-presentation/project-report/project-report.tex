...
Rectified Linear unit or `Relu` is the most widely used activation, it cuts off the negative values. It is defined as $Relu(x) = \max(0, x)$. \\
\centerline{\includegraphics[height=6cm]{../../docs/relu.png}}

Sigmoid is an other widely used function, it maps every values between 0 and 1. It is defined as $Sigmoid(x) = \frac{1} {1 + e^{-x}}$ \\
\centerline{\includegraphics[height=6cm]{../../docs/sigmoid.png}}

Tanh, similarly to Sigmoid maps every values between -1 and 1. $ tanh(x) = \frac{1 - e^{-2x}}{1 + e^{-2x}}$ \\
