...
The goal of these parts is to manipulate datat set. We take the information that the camera is giving to us, and we can load, save and sort those datas in a cleaner way so that our training model and our Artificial intelligence model can use it.\\ 
...
Thirdly, we have to test if our functions are capable to sort our dataset, by naming our files with different dates we can check if after the function load sorted datase we have a list of dictionaries in increasing order. \\ 
